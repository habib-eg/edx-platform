        ...
        for student in students:
            ...
            try:
                ...
                if _check_if_learner_on_blocklist(course_key, user):
                    ...
                    log.warning('Student %s is blocked from receiving a Certificate in Course %s',
                ...
                # make sure user isn't already on the exception list
                    ...
                    log.warning(u'Student %s already on exception list in Course %s.', user.username, course_key)
                # make sure user is enrolled in course
                elif not CourseEnrollment.is_enrolled(user, course_key):
                    build_row_errors('user_not_enrolled', user, row_num)
                    log.warning(u'Student %s is not enrolled in Course %s', user.username, course_key)
                else:
